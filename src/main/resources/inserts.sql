...
CREATE TABLE Attraction (id SERIAL NOT NULL, title varchar(30) NOT NULL, price float4, description varchar(2000) NOT NULL, fk_program_id bigint unsigned NOT NULL, fk_institution_id bigint unsigned, PRIMARY KEY (id), CONSTRAINT description_min_length CHECK (length(description) >= 100), CONSTRAINT non_negative_price CHECK (price >= 0));
